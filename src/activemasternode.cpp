...
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        // allow masternodes on different ports, same host
        /*if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (service.GetPort() != 41798) {
                notCapableReason = strprintf("Invalid port: %u - only 41798 is supported on mainnet.", service.GetPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if (service.GetPort() == 41798) {
            notCapableReason = strprintf("Invalid port: %u - 41798 is only supported on mainnet.", service.GetPort());
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }*/

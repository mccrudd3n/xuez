...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-checkblocks=<n>", strprintf(_("How many blocks to check at startup (default: %u, 0 = all)"), 1));
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "xuez.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "xuezd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the XUEZ and zXUEZ money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 41798, 51474));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in XUEZ/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in XUEZ/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, xuez, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in XUEZ/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Xuez specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:41798"));
    ...
    strUsage += HelpMessageOpt("-backupzXUEZ=<n>", strprintf(_("Enable automatic wallet backups triggered after each zXUEZ minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizexuezamount=<n>     " + strprintf(_("Keep N XUEZ anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Xuez Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Xuez Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Xuez Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Xuez: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZXUEZMinted();
                        RecalculateZXUEZSpent();
                    }
                    RecalculateXUEZSupply(1);
                }
                ...
                // Xuez: recalculate Accumulator Checkpoints that failed to database properly
                ...
                int nDefaultCheck = ( chainActive.Height() > Params().LAST_POW_BLOCK() ) ? 1 : 10;
                LogPrintf("Checking last %d blocks at startup.\n", nDefaultCheck);
				if (!CVerifyDB().VerifyDB(pcoinsdbview, 4, GetArg("-checkblocks", nDefaultCheck))) {
                //if (!CVerifyDB().VerifyDB(pcoinsdbview, 4, GetArg("-checkblocks", 1))) {
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Xuez Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Xuez Core to complete") << "\n";
        ...
        bool fEnableZXUEZBackups = GetBoolArg("-backupzXUEZ", true);
        pwalletMain->setZXUEZAutoBackups(fEnableZXUEZBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeXuezAmount = GetArg("-anonymizexuezamount", 0);
//    if (nAnonymizeXuezAmount > 999999) nAnonymizeXuezAmount = 999999;
//    if (nAnonymizeXuezAmount < 2) nAnonymizeXuezAmount = 2;
    ...
    LogPrintf("Anonymize Xuez Amount %d\n", nAnonymizeXuezAmount);
    ...
    /* Denominations
       ...
       For example:
       1XUEZ+1000 == (.1XUEZ+100)*10
       10XUEZ+10000 == (1XUEZ+1000)*10
    */

...
{
    //Xuez will hash in the transaction hash and the index number in order to make sure each hash is unique
...
bool CheckStakeKernelHash(unsigned int nBits, const CBlock blockFrom, const CTransaction txPrev, const COutPoint prevout, unsigned int& nTimeTx, unsigned int nHashDrift, bool fCheck, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
	...
	//if (nValueIn < Params().MinStakeInput())
			//return error("CheckStakeKernelHash() : min input violation - nValueIn=%d minStakeInput=%d", nValueIn, Params().MinStakeInput());

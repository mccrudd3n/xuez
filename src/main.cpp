...
unsigned int nStakeMinAge = 60 * 60 * 6; // * 6  for lauch, 6hrs;
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zXUEZ spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zXUEZ spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zXUEZ spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue( int nHeight )
{
	int64_t nSubsidy = 0;
	if ( nHeight < 10 ){
		nSubsidy = 244621.01 * COIN; // Premine used for XIOS swap ( 2,446,210 XUEZ )
	} else if ( nHeight <= 500 ) { // swap distribution / team masternode testing
		nSubsidy = 0.01 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Xuez
        ...
        // specially with outsEmpty.
        {
            ...
            if (*outs != outsBlock)
                fClean = fClean && error("DisconnectBlock() : added transaction mismatch? database corrupted, version %d, block %d", outsBlock.nVersion, pindex->nHeight);
...
{
    RenameThread("xuez-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZXUEZMinted()
{
...
void RecalculateZXUEZSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zXUEZ supply
...
bool RecalculateXUEZSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zXUEZ with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zXUEZSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert XUEZ to zXUEZ
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%u unixtime=%d\n",
        chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), log(chainActive.Tip()->nChainWork.getdouble()) / log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
        Checkpoints::GuessVerificationProgress(chainActive.Tip()), (unsigned int)pcoinsTip->GetCacheSize(), chainActive.Tip()->GetBlockTime());
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zXUEZ spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zXUEZ serial %s in block\n Block: %s",
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    // Reject block.nVersion=1,2,3 after PoW ends
    if ( block.nVersion < 4 && nHeight > Params().LAST_POW_BLOCK() ) {
        return state.Invalid(error("%s : rejected nVersion=%d block %d", __func__, block.nVersion, nHeight), REJECT_OBSOLETE, "bad-version");
    }
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zXUEZ mints and %d zXUEZ spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Xuez: We use certain sporks during IBD, so check to see if they are

...
int GetBudgetPaymentCycleBlocks()
{
    ...
    if( chainActive.Height() > 500 && Params().NetworkID() == CBaseChainParams::MAIN) return 43200;
...
{
    int nHighestCount = -1;
    int nTwentyPercent = mnodeman.CountEnabled(ActiveProtocol()) / 5;
    ...
    LogPrint("masternode","CBudgetManager::IsBudgetPaymentBlock() - nHighestCount: %lli, 20%% of Masternodes: %lli. Number of budgets: %lli\n", 
              nHighestCount, nTwentyPercent, mapFinalizedBudgets.size());

    // If budget doesn't have 5% of the network votes, then we should pay a masternode instead
    if (nHighestCount > nTwentyPercent) return true;
...
bool CBudgetManager::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    LogPrint("masternode","CBudgetManager::IsTransactionValid() - nHighestCount: %lli, 20%% of Masternodes: %lli mapFinalizedBudgets.size(): %ld\n", 
              nHighestCount, nTwentyPercent, mapFinalizedBudgets.size());
    /*
        If budget doesn't have 20% of the network votes, then we should pay a masternode instead
    */
    if (nHighestCount < nTwentyPercent) return false;

    // check the highest finalized budgets (+/- 25% to assist in consensus)

    it = mapFinalizedBudgets.begin();
    while (it != mapFinalizedBudgets.end()) {
        CFinalizedBudget* pfinalizedBudget = &((*it).second);

        if (pfinalizedBudget->GetVoteCount() > nHighestCount - mnodeman.CountEnabled(ActiveProtocol()) / 4) {
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    if ( nHeight <  500 || chainActive.Tip() == NULL ) return 0; // swap premine
    ...
    int duration = GetBudgetPaymentCycleBlocks();
    return ( nSubsidy / 10 ) * duration;
...
bool CBudgetProposal::IsValid(std::string& strError, bool fCheckCollateral)
{
    ...
    if (nAmount < 10 * COIN) {
        strError = "Proposal " + strProposalName + ": Invalid nAmount nAmount: " + to_string( nAmount/COIN ) + ", 10 * COIN: " + to_string( 10 * COIN ) +", diff: " + to_string( ( 10 * COIN )  - nAmount / COIN ) + ", height:" + to_string( chainActive.Height() );
        return false;
    }

...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::XUEZ);
    ...
    if (!settings.contains("nAnonymizeXuezAmount"))
        settings.setValue("nAnonymizeXuezAmount", 1000);

    nAnonymizeXuezAmount = settings.value("nAnonymizeXuezAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeXuezAmount"))
        SoftSetArg("-anonymizexuezamount", settings.value("nAnonymizeXuezAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in xuez.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeXuezAmount:
            return QVariant(nAnonymizeXuezAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeXuezAmount:
            nAnonymizeXuezAmount = value.toInt();
            settings.setValue("nAnonymizeXuezAmount", nAnonymizeXuezAmount);
            emit anonymizeXuezAmountChanged(nAnonymizeXuezAmount);
            break;
        case CoinControlFeatures:

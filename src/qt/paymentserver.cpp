...
const char* BIP71_MIMETYPE_PAYMENT = "application/xuez-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/xuez-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/xuez-paymentrequest";
...
//
// OSX-specific way of handling xuez: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject* object, QEvent* event)
{
    // clicking on xuez: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in xuez: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // xuez: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach (const PAIRTYPE(CScript, CAmount) & sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom xuez addresses are not supported

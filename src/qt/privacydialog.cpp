                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXUEZ ought to be enough for anybody." - Bill Gates, 2017
    ui->zXUEZpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXUEZSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzXUEZ->setEnabled(false);
        ui->pushButtonMintzXUEZ->setToolTip(tr("zXUEZ is currently disabled due to maintenance."));

        ui->pushButtonSpendzXUEZ->setEnabled(false);
        ui->pushButtonSpendzXUEZ->setToolTip(tr("zXUEZ is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXUEZpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXUEZ_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXUEZ is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXUEZ...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXUEZ in ") + 
...
void PrivacyDialog::on_pushButtonSpendzXUEZ_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXUEZ is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zXUEZ
        sendzXUEZ();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXUEZ
    sendzXUEZ();
}

void PrivacyDialog::on_pushButtonZXUEZControl_clicked()
{
    ZXUEZControlDialog* zXUEZControl = new ZXUEZControlDialog(this);
    zXUEZControl->setModel(walletModel);
    zXUEZControl->exec();
}

void PrivacyDialog::setZXUEZControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXUEZSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXUEZ()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Xuez Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXUEZpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXUEZ is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XUEZ </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXUEZ</b>";
    ...
    // use mints from zXUEZ selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXUEZControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZXUEZControlDialog::GetSelectedMints();
    }
    ...
    // Clear zXUEZ selector in case it was used
    ZXUEZControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXUEZ Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " XUEZ, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zXUEZ Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXUEZ </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXUEZ "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXUEZ "));
    ui->labelzXUEZAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzXUEZSyncStatus->setVisible(fShow);
}

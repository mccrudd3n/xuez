...
std::list<std::string> ZXUEZControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZXUEZControlDialog::listMints;

ZXUEZControlDialog::ZXUEZControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZXUEZControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZXUEZControlDialog::~ZXUEZControlDialog()
{
    delete ui;
}

void ZXUEZControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXUEZControlDialog::updateList()
{
...
void ZXUEZControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXUEZControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZXUEZ_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXUEZControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZXUEZControlDialog::GetSelectedMints()
{
...
void ZXUEZControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

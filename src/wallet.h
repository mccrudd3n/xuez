...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 XUEZ at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zXUEZ send
enum ZerocoinSpendStatus {
    ZXUEZ_SPEND_OKAY = 0,                            // No error
    ZXUEZ_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXUEZ_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXUEZ_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXUEZ_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXUEZ_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXUEZ_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXUEZ_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXUEZ_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXUEZ_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXUEZ_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXUEZ_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXUEZ_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXUEZ_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXUEZ_SPENT_USED_ZXUEZ = 14,                      // Coin has already been spend
    ZXUEZ_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZXUEZAutoBackups(bool fEnabled)
    {

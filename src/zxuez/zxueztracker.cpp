...
#include <zxuez/deterministicmint.h>
#include "zxueztracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
#include "wallet/walletdb.h"
#include "zxuez/accumulators.h"
#include "zxuez/zxuezwallet.h"
#include "witness.h"


CzXUEZTracker::CzXUEZTracker(std::string strWalletFile)
{
...
CzXUEZTracker::~CzXUEZTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzXUEZTracker::Init()
{
...
bool CzXUEZTracker::Archive(CMintMeta& meta)
{
...
bool CzXUEZTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzXUEZTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzXUEZTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzXUEZTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzXUEZTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzXUEZTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzXUEZTracker::GetSerialHashes()
{
...
CAmount CzXUEZTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzZXUEZModuleTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzZXUEZModuleTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzZXUEZModuleTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzZXUEZModuleTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzZXUEZModuleTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzZXUEZModuleTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzZXUEZModuleTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzZXUEZModuleTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzZXUEZModuleTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzZXUEZModuleTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzZXUEZModuleWallet* zZXUEZModuleWallet)
{
    bool iszZXUEZModuleWalletInitialized = (NULL != zZXUEZModuleWallet);
    CMintMeta meta;
    ...
    if (! iszZXUEZModuleWalletInitialized)
        zZXUEZModuleWallet = new CzZXUEZModuleWallet(strWalletFile);
    meta.isSeedCorrect = zZXUEZModuleWallet->CheckSeed(dMint);
    if (! iszZXUEZModuleWalletInitialized)
        delete zZXUEZModuleWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzZXUEZModuleTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzZXUEZModuleTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzZXUEZModuleTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzZXUEZModuleTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzZXUEZModuleTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzZXUEZModuleTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzZXUEZModuleWallet* zZXUEZModuleWallet = new CzZXUEZModuleWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zZXUEZModuleWallet);
        }
        delete zZXUEZModuleWallet;
        LogPrint("zero", "%s: added %d dzxuez from DB\n", __func__, listDeterministicDB.size());
    }
...
void CzZXUEZModuleTracker::Clear()
{
    mapSerialHashes.clear();
}

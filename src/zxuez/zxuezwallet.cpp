...
#include "zxuezchain.h"


CzXUEZWallet::CzXUEZWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zxuez seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXUEZSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXUEZSeed_deprecated()) {
                    LogPrintf("%s: Updated zXUEZ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zxuez seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zxuez wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXUEZWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXUEZCount(nCountLastUsed);
    else if (!walletdb.ReadZXUEZCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXUEZWallet::Lock()
{
    seedMaster = 0;
}

void CzXUEZWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXUEZWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXUEZ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXUEZWallet::LoadMintPoolFromDB()
{
...
void CzXUEZWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXUEZWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXUEZWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zxuezTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXUEZWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXUEZ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zxuezTracker which also adds to database
    pwalletMain->zxuezTracker->Add(dMint, true);
...
void CzXUEZWallet::SeedToZXUEZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXUEZWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXUEZWallet::GenerateDeterministicZXUEZ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXUEZWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXUEZWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzXUEZWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
